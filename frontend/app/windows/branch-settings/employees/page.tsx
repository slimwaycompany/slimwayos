...
interface TempPasswordModal { email: string; password: string; }
  ...
  return (
    <div className="p-6 max-w-sm">
      ...
      <form onSubmit={handleCreate} className="flex flex-col gap-4">
        ...
        {error && <div className="rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3 text-body text-red-400">{error}</div>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="glass flex w-[480px] flex-col gap-4 p-8 shadow-2xl">
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => setModal(null)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-white/10 hover:text-white">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="rounded-xl bg-yellow-500/10 border border-yellow-500/20 px-4 py-3">
              <p className="text-caption text-yellow-400 font-semibold">Запишите и передайте сотруднику — пароль показывается один раз</p>
            </div>
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                <code className="rounded-lg bg-gray-800 px-4 py-2 text-body font-mono text-teal tracking-widest">{modal.password}</code>
              </div>
            </div>
