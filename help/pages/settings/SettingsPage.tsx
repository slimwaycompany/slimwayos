...
import { User, Palette, Bell, Volume2, VolumeX, Play, Check, Shield, Copy, QrCode, X, RefreshCw, Home } from 'lucide-react'
...
function AutoRefreshSection() {
  const [interval, setInterval_] = useState<number>(() => {
    try { return Number(localStorage.getItem('autoRefreshInterval') ?? '0') } catch { return 0 }
  })

  const save = (v: number) => {
    setInterval_(v)
    try { localStorage.setItem('autoRefreshInterval', String(v)) } catch { /* ignore */ }
  }

  return (
    <Section title="Автообновление" icon={<RefreshCw size={15} strokeWidth={1.75} color="var(--accent)" />}>
      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 13 }}>
        Автоматическое обновление данных в браузере
      </div>
        ...
        {AUTO_REFRESH_OPTIONS.map(opt => (
          <button
            ...
            style={{
              ...
              border: interval === opt.value ? '1px solid var(--accent)' : '1px solid var(--border)',
              background: interval === opt.value ? 'color-mix(in srgb, var(--accent) 10%, transparent)' : 'transparent',
              color: interval === opt.value ? 'var(--accent)' : 'var(--text)',
              fontSize: 12, fontWeight: interval === opt.value ? 600 : 400,
              cursor: 'pointer',
...
const ALL_PAGES = [
  { path: '/dashboard',     label: 'Дашборд',    resource: null,           action: null },
  { path: '/clients',       label: 'Клиенты',    resource: 'clients',      action: 'view' },
  { path: '/subscriptions', label: 'Абонементы', resource: 'subscriptions', action: 'view' },
  { path: '/sale',          label: 'Продажа',    resource: 'subscriptions', action: 'create' },
  { path: '/schedule',      label: 'Расписание', resource: 'schedule',     action: 'view' },
  { path: '/leads',         label: 'Лиды',       resource: 'leads',        action: 'view' },
  { path: '/tasks',         label: 'Задачи',     resource: 'tasks',        action: 'view' },
  { path: '/chat',          label: 'Чат',        resource: null,           action: null },
  { path: '/employees',     label: 'Сотрудники', resource: 'employees',    action: 'view' },
  { path: '/schedule-work', label: 'График',     resource: 'shifts',       action: 'view' },
  { path: '/timesheet',     label: 'Табель',     resource: 'timesheet',    action: 'view' },
  { path: '/warehouse',     label: 'Склад',      resource: 'warehouse',    action: 'view' },
  { path: '/payroll',       label: 'Зарплата',   resource: 'employees',    action: 'view' },
  { path: '/management',    label: 'Управление', resource: 'management',   action: 'view' },
  { path: '/settings',      label: 'Настройки',  resource: null,           action: null },
]

function StartPageSection() {
  const perm = usePermissions()
  const [startPage, setStartPage] = useState<string>(() => {
    try { return localStorage.getItem('startPage') ?? '/dashboard' } catch { return '/dashboard' }
  })

  const save = (path: string) => {
    setStartPage(path)
    try { localStorage.setItem('startPage', path) } catch { /* ignore */ }
  }

  const available = ALL_PAGES.filter(p => {
    if (!p.resource || !p.action) return true
    return perm.can(p.resource as Parameters<typeof perm.can>[0], p.action as Parameters<typeof perm.can>[1])
  })

  return (
    <Section title="Главный экран" icon={<Home size={15} strokeWidth={1.75} color="var(--accent)" />}>
      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 13 }}>
        Страница, на которую вы попадёте после входа
      </div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {available.map(page => (
          <button
            ...
            style={{
              height: 32, padding: '0 13px',
              borderRadius: 'var(--radius-md)',
              border: startPage === page.path ? '1px solid var(--accent)' : '1px solid var(--border)',
              background: startPage === page.path ? 'color-mix(in srgb, var(--accent) 10%, transparent)' : 'transparent',
              color: startPage === page.path ? 'var(--accent)' : 'var(--text)',
              fontSize: 12, fontWeight: startPage === page.path ? 600 : 400,
              cursor: 'pointer',
              transition: 'border-color 150ms ease-out, background 150ms ease-out, color 150ms ease-out',
            }}
          >
