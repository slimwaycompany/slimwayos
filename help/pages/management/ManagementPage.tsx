...
import {
  ...
  Globe, Zap, Clock, Copy, QrCode, Key, Webhook, Radio,
  Camera, Music2, Send, Phone, PlayCircle, ExternalLink,
} from 'lucide-react'
...
const TRIGGER_LABELS: Record<string, string> = {
  ...
  subscription_expiring_pipeline: 'Абонемент истекает через N дней → pipeline',
  subscription_expired:          'Абонемент истёк → pipeline',
  subscription_purchased:        'Клиент купил абонемент → pipeline',
}
const TRIGGER_ICONS: Record<string, React.ReactNode> = {
  lead_created:                  <Zap size={15} color="var(--accent)" />,
  lead_no_activity:              <Clock size={15} color="var(--color-warning)" />,
  subscription_expiring:         <AlertCircle size={15} color="var(--color-warning)" />,
  subscription_expiring_pipeline: <AlertCircle size={15} color="var(--color-warning)" />,
  subscription_expired:          <AlertCircle size={15} color="var(--color-danger)" />,
  subscription_purchased:        <Zap size={15} color="var(--color-success)" />,
}
...
function AutomationTab() {
  ...
  const handleSave = async () => {
    const isPipelineOnlyTrigger = ['subscription_expiring_pipeline', 'subscription_expired', 'subscription_purchased'].includes(trigger)
    if (!isPipelineOnlyTrigger && !titleTemplate.trim()) { setFormError('Введите шаблон заголовка'); return }
    setSaving(true); setFormError(null)
    const needsDaysPayload = ['lead_no_activity', 'subscription_expiring', 'subscription_expiring_pipeline'].includes(trigger)
    const payload = {
  ...
  const needsDays = ['lead_no_activity', 'subscription_expiring', 'subscription_expiring_pipeline'].includes(trigger)
  const pipelineOnlyTriggers = ['subscription_expiring_pipeline', 'subscription_expired', 'subscription_purchased']
  const isPipelineOnly = pipelineOnlyTriggers.includes(trigger)
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <select style={selectStyle} value={trigger} onChange={e => setTrigger(e.target.value)}>
                  ...
                  <option value="subscription_expiring">Абонемент истекает через N дней (задача)</option>
                  <option value="subscription_expiring_pipeline">Абонемент истекает через N дней → pipeline</option>
                  <option value="subscription_expired">Абонемент истёк → pipeline</option>
                  <option value="subscription_purchased">Клиент купил абонемент → pipeline</option>
                </select>
              </div>
              {needsDays && (
                <div>
...
const CHANNEL_PLATFORMS: { value: ChannelPlatform; label: string; icon: React.ReactNode; color: string }[] = [
  { value: 'instagram', label: 'Instagram', icon: <Camera size={14} />,     color: 'var(--color-danger)' },
  { value: 'tiktok',    label: 'TikTok',    icon: <Music2 size={14} />,     color: 'var(--text)' },
  { value: 'telegram',  label: 'Telegram',  icon: <Send size={14} />,       color: 'var(--color-info)' },
  { value: 'whatsapp',  label: 'WhatsApp',  icon: <Phone size={14} />,      color: 'var(--color-success)' },
  { value: 'youtube',   label: 'YouTube',   icon: <PlayCircle size={14} />, color: 'var(--color-danger)' },
]
...
function ChannelsSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { setChannels(await channelsApi.getAll()) }
    catch { /* ignore */ }
    finally { setLoading(false) }
  }, [])
  ...
  const openCreate = () => {
    setEditTarget(null); setPlatform('instagram'); setName(''); setUrl(''); setDescription(''); setFormError(null); setShowModal(true)
  }

  const openEdit = (c: Channel) => {
    setEditTarget(c); setPlatform(c.platform); setName(c.name); setUrl(c.url); setDescription(c.description ?? ''); setFormError(null); setShowModal(true)
  }

  const handleSave = async () => {
    if (!name.trim()) { setFormError('Введите название'); return }
    if (!url.trim()) { setFormError('Введите ссылку'); return }
    setSaving(true); setFormError(null)
    try {
      const payload: ChannelPayload = { platform, name: name.trim(), url: url.trim(), description: description.trim() || undefined }
      const result = editTarget ? await channelsApi.update(editTarget.id, payload) : await channelsApi.create(payload)
      if (editTarget) setChannels(prev => prev.map(x => x.id === result.id ? result : x))
      else setChannels(prev => [result, ...prev])
      setShowModal(false)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { error?: string } } })?.response?.data?.error
      setFormError(msg ?? 'Ошибка при сохранении')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{channels.length} каналов</div>
        <button onClick={openCreate} style={{ display: 'flex', alignItems: 'center', gap: 5, height: 32, padding: '0 12px', background: 'color-mix(in srgb, var(--accent) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 25%, transparent)', borderRadius: 8, color: 'var(--accent)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {channels.map(c => {
            const cfg = CHANNEL_PLATFORMS.find(p => p.value === c.platform)
            const color = cfg?.color ?? 'var(--accent)'
            return (
              <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', background: 'var(--bg-surface)', borderRadius: 10, border: '1px solid var(--border)' }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: `color-mix(in srgb, ${color} 10%, transparent)`, border: `1px solid color-mix(in srgb, ${color} 20%, transparent)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color, flexShrink: 0 }}>
                  {cfg?.icon}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{c.name}</div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>{cfg?.label}</div>
                </div>
                <a href={c.url} target="_blank" rel="noopener noreferrer"
                  style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--text-muted)', textDecoration: 'none', flexShrink: 0 }}>
                  <ExternalLink size={11} />
                </a>
                <button onClick={() => openEdit(c)} className="icon-btn"><Edit2 size={13} /></button>
                <button onClick={() => void handleDelete(c.id)} className="icon-btn" style={{ color: 'var(--color-danger)' }}><Trash2 size={13} /></button>
              </div>
            )
          })}
        </div>
      ...
      {showModal && ReactDOM.createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={() => setShowModal(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(6px)' }} />
          <div className="modal-animate" style={{ position: 'relative', width: '100%', maxWidth: 460, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, boxShadow: '0 24px 64px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
              <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>{editTarget ? 'Редактировать канал' : 'Добавить канал'}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {CHANNEL_PLATFORMS.map(p => (
                    <button key={p.value} type="button" onClick={() => setPlatform(p.value)} style={{
                      display: 'flex', alignItems: 'center', gap: 6, height: 32, padding: '0 10px',
                      background: platform === p.value ? `color-mix(in srgb, ${p.color} 10%, transparent)` : 'transparent',
                      border: `1px solid ${platform === p.value ? `color-mix(in srgb, ${p.color} 40%, transparent)` : 'var(--border)'}`,
                      borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: platform === p.value ? 600 : 400,
                      color: platform === p.value ? p.color : 'var(--text-muted)',
              ...
              <div>
                ...
                <input style={inputStyle} value={name} onChange={e => setName(e.target.value)} placeholder="Slimway Official" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea style={{ ...inputStyle, height: 60, paddingTop: 8, paddingBottom: 8, resize: 'none' as const }} value={description} onChange={e => setDescription(e.target.value)} placeholder="Краткое описание..." />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => void handleSave()} disabled={saving} className="btn btn-primary" style={{ flex: 1 }}>
...
type ManagementTab = 'general' | 'subscriptions' | 'catalog' | 'permissions' | 'users' | 'audit' | 'branch_settings' | 'booking' | 'automation' | 'api_keys' | 'webhooks' | 'channels'
...
export default function ManagementPage() {
  ...
  const TABS: { id: ManagementTab; label: string; icon: React.ReactNode }[] = [
    ...
    ...(isFranchiseeOrAbove ? [{ id: 'webhooks' as ManagementTab, label: 'Webhooks', icon: <Webhook size={14} strokeWidth={1.75} /> }] : []),
    ...(isFranchiseeOrAbove ? [{ id: 'channels' as ManagementTab, label: 'Каналы', icon: <Radio size={14} strokeWidth={1.75} /> }] : []),
  ]
  ...
  return (
    <div>
      ...
      {tab === 'webhooks' && isFranchiseeOrAbove && (
        <WebhooksPage />
      )}
      {tab === 'channels' && isFranchiseeOrAbove && (
        <div style={{ maxWidth: 700 }}>
          <Section title="Каналы" icon={<Radio size={15} strokeWidth={1.75} color="var(--accent)" />}>
            <ChannelsSection />
          </Section>
        </div>
      )}
