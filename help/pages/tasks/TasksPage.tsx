...
function TaskCardContent({ task, employees, onClick, onContextMenu, isDragging }: TaskCardProps) {
  ...
  return (
    ...
    >
      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', lineHeight: 1.4, letterSpacing: '-0.01em' }}>
        {task.title}
      </span>
      ...
      {(deadline || assignee) && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
          ...
          {assignee && (
            <div title={assignee.full_name} style={{
        ...
        <div style={{ padding: '16px 18px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 4, overflowX: 'auto' }}>
            {STANDARD_COLUMNS.map(col => {
              const isActive = col.id === normalizeStatus(task.status)
              return (
                <button
                  key={col.id}
                  onClick={() => void handleStatusChange(col.id)}
                  style={{
                    flex: 1, minWidth: 'max-content', height: 24, padding: '0 10px',
              ...
              <div>
                ...
                {canEdit ? (
                  <textarea
                    ...
                    style={{ padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box', fontFamily: 'inherit', resize: 'vertical', lineHeight: 1.5, minHeight: 72 }}
...
function CreateTaskModal({ employees, projects, defaultStatus, onClose, onCreate }: {
  ...
  const [checklistGroups,  setChecklistGroups]  = useState<Array<{ id: string; name: string; items: string[]; newItem: string }>>([])
  const [saving,           setSaving]           = useState(false)
  const [error,       setError]       = useState<string | null>(null)
  const [showMore,    setShowMore]    = useState(false)

  const toggleObs     = (pid: string) => setObservers(prev => prev.includes(pid) ? prev.filter(id => id !== pid) : [...prev, pid])
  const addCheckGroup = () => setChecklistGroups(prev => [...prev, { id: crypto.randomUUID(), name: '', items: [], newItem: '' }])
  const updateGroup   = (id: string, patch: Partial<{ name: string; items: string[]; newItem: string }>) =>
    setChecklistGroups(prev => prev.map(g => g.id === id ? { ...g, ...patch } : g))
  const addGroupItem  = (id: string) =>
    setChecklistGroups(prev => prev.map(g => {
      if (g.id !== id || !g.newItem.trim()) return g
      return { ...g, items: [...g.items, g.newItem.trim()], newItem: '' }
    }))
  const removeGroupItem = (groupId: string, i: number) =>
    setChecklistGroups(prev => prev.map(g => g.id === groupId ? { ...g, items: g.items.filter((_, j) => j !== i) } : g))
  const removeGroup   = (id: string) => setChecklistGroups(prev => prev.filter(g => g.id !== id))
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      for (const g of checklistGroups) {
        ...
        const group = await tasksApi.addChecklistGroup(task.id, g.name.trim() || 'Чеклист')
        for (const text of g.items) await tasksApi.addChecklistItem(task.id, text, group.id)
      }
      const full = checklistGroups.some(g => g.items.length > 0) ? await tasksApi.getById(task.id) : task
      onCreate(full)
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              <label style={labelStyle}>Описание</label>
              <textarea
                style={{ padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box', fontFamily: 'inherit', resize: 'vertical', lineHeight: 1.5, minHeight: 60 }}
            ...
            >
              ...
              {(observers.length > 0 || checklistGroups.some(g => g.items.length > 0)) && (
                <span style={{ marginLeft: 'auto', background: 'var(--accent)', color: 'var(--accent-fg)', fontSize: 10, borderRadius: 10, padding: '1px 6px', fontVariantNumeric: 'tabular-nums' }}>
                  {observers.length + checklistGroups.reduce((s, g) => s + g.items.length, 0)}
                </span>
              )}
            </button>
              ...
              <div style={{ padding: '12px', background: 'var(--bg)', borderRadius: 8, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div>
                  ...
                  {checklistGroups.map(g => (
                    <div key={g.id} style={{ marginBottom: 8, border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
                        <input
                          style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontSize: 12, fontWeight: 600, color: 'var(--text)', fontFamily: 'inherit' }}
                          value={g.name}
                          onChange={e => updateGroup(g.id, { name: e.target.value })}
                          placeholder="Название чеклиста"
                        />
                        <button onClick={() => removeGroup(g.id)} className="icon-btn" style={{ width: 18, height: 18 }}><X size={11} /></button>
                      </div>
                      <div style={{ padding: '4px 8px' }}>
                        {g.items.map((text, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 4px' }}>
                            <span style={{ width: 11, height: 11, borderRadius: 3, border: '1px solid var(--border)', flexShrink: 0 }} />
                            <span style={{ flex: 1, fontSize: 12, color: 'var(--text)' }}>{text}</span>
                            <button onClick={() => removeGroupItem(g.id, i)} className="icon-btn" style={{ width: 16, height: 16 }}><X size={10} /></button>
                          </div>
                        ))}
                        <div style={{ display: 'flex', gap: 5, marginTop: 3, paddingBottom: 4 }}>
                          <input
                            style={{ ...inputStyle, height: 28, fontSize: 12 }}
                            value={g.newItem}
                            onChange={e => updateGroup(g.id, { newItem: e.target.value })}
                            onKeyDown={e => { if (e.key === 'Enter') addGroupItem(g.id) }}
                            placeholder="Новый пункт..."
                          />
                          <button onClick={() => addGroupItem(g.id)} style={{ height: 28, width: 28, background: 'color-mix(in srgb, var(--color-success) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--color-success) 25%, transparent)', borderRadius: 7, color: 'var(--color-success)', fontSize: 16, cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                        </div>
                      </div>
                    </div>
                  ))}
                  <button onClick={addCheckGroup} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, height: 28, padding: '0 10px', background: 'transparent', border: '1px dashed var(--border)', borderRadius: 7, color: 'var(--text-muted)', fontSize: 12, cursor: 'pointer', width: '100%', marginTop: 2 }}>
                    <Plus size={11} />Добавить чеклист
                  </button>
                </div>
              </div>
            )}
